...
#
#               Additional patient records are created from HLA and virus sequencing datasets, where non-KGH patients are added.
#
...
import config
from .acute_patients import clean_lassa_acute
from .survivors import clean_survivors
import helpers

# Main function to stitch together the patients.


def compile_patients(input_acuteid_roster, input_lassaAcute_ids, input_lassaAcute, input_survivor_ids, input_ebolaSurvivor,
                     patient_cols, expt_cols, dict_cols, dateModified, updatedBy, version, verbose):
    print('Compiling CViSB patients...')

    # --- acute Lassa ---
    # (1) call acute_patients to get the acute IDs
    acuteLassa = clean_lassa_acute(input_lassaAcute, input_acuteid_roster)
    ...
    survivorsAll = clean_survivors(input_survivor_ids, input_ebolaSurvivor, updatedBy, dateModified, version)

    # --- leftover IDs ---
    # (4) call additional ids -- extra ones that don't appear in other places.
    # extra = clean_extra_ids(input_acuteid_roster)
    ...
    # df2export[patient_cols].to_json(output_patients + ".json", orient="records")
    # df2export[patient_cols].sample(25).to_json(
    #     output_patients + "_randomsample.json", orient="records")
    # df2export[patient_cols].to_csv(output_patients + "_clean.csv", index=False)
    # df2export[patient_cols].to_csv(output_patients + ".csv", index = False)
    ...
    return({"patient": merged, "sample": pd.DataFrame(), "dataset": pd.DataFrame(), "datadownload": pd.DataFrame(), "experiment": pd.DataFrame()})
...
def cleanup_merged(df, dateModified):
    ...
    df['correction'] = None  # not used now...
...
if __name__ == '__main__':
    compile_patients(config.output_patients, config.input_acuteid_roster, config.output_extraIDs_weirdos, config.input_survivor_ids, config.output_allSurvivors, config.output_survivorRoster, config.output_survivorRosterWeirdos,
                     ...
                     config.output_lassaAcuteWeirdos, config.output_lassaAcuteWeirdos_ids, config.patient_cols, config.dict_cols)
