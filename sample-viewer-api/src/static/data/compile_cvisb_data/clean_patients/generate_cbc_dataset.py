...
def get_cbc_dataset(dateModified, downloadIDs, experiments, countries, version, dataset_id):
    ds = {}
    ...
    ds["name"] = "Blood Cell Counts of Ebola/Lassa Patients"

    ds["includedInDataCatalog"] = ["https://data.cvisb.org/"]
    ds["url"] = f"https://data.cvisb.org/dataset/{dataset_id}"

    # descriptions
    ds["description"] = "Blood cell count measurement data of suspected or confirmed Ebola or Lassa Fever patients in the acute phase of infection."
    ds["keywords"] = ["blood cell count", "comlete blood cell count",
    "whole blood count", "hemoglobin", "platelets",
    "neutrophils", "bands", "lymphocytes", "monocytes", "eosinophils",
    "basophils", "prothrombin time", "international normalization ratio", "dDimer",
                      "Ebola",
                      "Ebola virus",
                      "EBOV",
                      "Lassa",
...
# --- possibly variable, each time ---
    ...
    # data downloads
    ds["dataDownloadIDs"] = ds["dataDownloadIDs"] = helpers.getUnique(downloadIDs, "identifier")
